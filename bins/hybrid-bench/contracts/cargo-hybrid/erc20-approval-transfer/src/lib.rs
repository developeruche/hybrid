...
impl ERC20 {
    ...
    pub fn Benchmark(&mut self, n: U256)  -> Result<U256, ERC20Error> {
        let _ = self.mint(msg_sender(), U256::from_str_radix("1000000000000000000000000000", 10).unwrap_or_default());
        let nn = n.into_limbs()[0];
        for i in 1..nn {
            if self.allowance(msg_sender(), msg_sender()) != U256::ZERO {
                return Err(ERC20Error::NonZeroAllowance);
            }
            let _ = self.approve(msg_sender(), U256::from(i));
            let _ = self.transfer_from(msg_sender(), msg_sender(), U256::from(i));
            let _ = self.allowance(msg_sender(), msg_sender());
        }
        
        Ok(self.balance_of(msg_sender()))
    }
